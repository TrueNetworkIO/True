...
pub mod pallet {
	...
	#[pallet::storage]
	pub type Algorithms<T: Config> = StorageMap<
		_,
		Blake2_128Concat,
		u64 /*algoId*/,
		Algorithm<T>,
	...
	#[pallet::type_value]
	pub fn DefaultNextAlgoId<T: Config>() -> u64 {
		100u64
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn save_algo(
			...
			let who = ensure_signed(origin)?;
				...
				schema_hashes.len() <= (T::MaxSchemas::get() as usize),
				Error::<T>::TooManySchemas
			);
			...
			Algorithms::<T>::insert(id, Algorithm {
				schema_hashes: BoundedVec::try_from(schema_hashes).map_err(
					|_| Error::<T>::TooManySchemas
				)?,
		...
		pub fn run_algo_for(
			...
			let mut attestations: Vec<credentials::CredAttestation<T>> = Vec::<>::with_capacity(
				algorithm.schema_hashes.len()
			);
			...
			for schema_hash in &algorithm.schema_hashes {
				...
				let attestations_for_schema = Attestations::<T>
				...
				// Filter and get all valid attestations
				let valid_attestations = credentials::Pallet::<T>::filter_attestations(
					attestations_for_schema,
					&schema
				);

				// Check if we have any valid attestations after filtering
				ensure!(!valid_attestations.is_empty(), Error::<T>::AttestationNotFound);

				// Get the latest valid attestation
				let latest_attestation = valid_attestations
					.last()
					.ok_or(Error::<T>::AttestationNotFound)?;
			...
			return Self::run_code(algorithm.code.to_vec(), attestations, algorithm.gas_limit);
		}
	}
	...
	impl<T: Config> Pallet<T> {
		pub fn run_code(
			code: Vec<u8>,
			attestations: Vec<CredAttestation<T>>,
			gas_limit: u64
		) -> DispatchResult {
			let engine = wasmi::Engine::default();
			...
			let host_print = wasmi::Func::wrap(
				...
				|mut caller: wasmi::Caller<'_, GasMeter>, param: i32| {
					caller
						.data_mut()
						.charge(T::GasCost::get().basic_op)
						.map_err(|_| Trap::new("Gas charge failed"))?;
			...
			let memory = wasmi::Memory
				::new(
					&mut store,
					wasmi::MemoryType
						::new(T::MaxMemoryPages::get(), Some(T::MaxMemoryPages::get()))
			...
			store
				.data_mut()
				.charge(T::GasCost::get().memory_op * ((bytes.len() as u64) / 32 + 1))
